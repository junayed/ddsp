...
def get_latest_operative_config(restore_dir):
  ...
  files = tf.io.gfile.listdir(restore_dir)
  configs = [f for f in files if 'operative_config' in f]
  iters = [int(c.split('-')[-1].split('.gin')[0]) for c in configs]
  max_idx = tf.argmax(iters)
  return os.path.join(restore_dir, configs[max_idx])
